...
function drawHeartShape() {
  ...
  const heartInterval = setInterval(() => {
    ...
    if (step >= totalSteps) {
      ...
      map.flyTo([midLat - 0.5, midLng], 6, { duration: 2.2 });

      setTimeout(() => document.getElementById('quote-1').classList.add('visible'), 1000);
      setTimeout(() => document.getElementById('quote-2').classList.add('visible'), 2800);
      setTimeout(() => document.getElementById('poem').classList.add('visible'), 4600);
      setTimeout(() => document.getElementById('next-btn').classList.remove('hidden'), 6200);
    }
  }, 25);
}
...
function triggerLockError() {
  ...
  // Affiche l'écran d'attaque / hack plein écran
  hackOverlay.classList.remove('hidden');

  // Laisse le piratage 3.5 secondes puis bascule vers l'avion
  setTimeout(() => {
